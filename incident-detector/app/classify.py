...
def classify_pod(pod: dict) -> Optional[Finding]:
    ...
    if service in ("unknown", "incident-detector"):
        return None
    ...
    if waiting_reason in ("ImagePullBackOff", "ErrImagePull"):
        return Finding(
            ...
            pod=name,
            message=f"kubelet cannot pull the image ({waiting_reason})",
        )
    ...
    if terminated_reason == "OOMKilled":
        return Finding(
            ...
            problem="OOMKilled",
            pod=name,
            message="container exceeded memory limit and was killed by the kernel",
        )
    ...
    if waiting_reason == "CrashLoopBackOff":
        return Finding(
            ...
            pod=name,
            message="container keeps exiting and Kubernetes is backing off restarts",
        )
    ...
    if phase == "Running" and statuses and not ready:
        return Finding(
            ...
            problem="NotReady",
            pod=name,
            message="pod is Running but not Ready (readiness probe failing or container not ready)",
        )
